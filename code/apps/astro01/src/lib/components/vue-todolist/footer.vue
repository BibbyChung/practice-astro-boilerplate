<template>
  <footer class="footer">
    <span class="todo-count">
      <template v-if="uncompletedCountRef === 1"> 1 uncompleted item left </template>
      <template v-else> {{ uncompletedCountRef }} uncompleted items left </template>
    </span>
    <ul class="filters">
      <li>
        <a
          @click.prevent="setTodosFilterBtn$.next('all')"
          :class="{ selected: todoFilterRef === 'all' }"
          href="#/"
          >All</a
        >
      </li>
      <li>
        <a
          @click.prevent="setTodosFilterBtn$.next('active')"
          :class="{ selected: todoFilterRef === 'active' }"
          href="#/"
          >Active</a
        >
      </li>
      <li>
        <a
          @click.prevent="setTodosFilterBtn$.next('completed')"
          :class="{ selected: todoFilterRef === 'completed' }"
          href="#/"
          >Completed</a
        >
      </li>
    ...
    <div>
      <button
        v-if="isShowClearCompletedRef"
        @click.prevent="removeAllTodosBtn$.next(true)"
...
const isShowClearCompletedRef = toRef(
  getTodos().pipe(
    map((todos) => {
...
const uncompletedCountRef = toRef(
